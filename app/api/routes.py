...
async def get_prices(db: DBSession, ticker: Ticker, limit: Limit = 100, offset: Offset = 0):
    """Возвращает список цен для указанного тикера"""
    return await service.get_prices(db, ticker, limit, offset)


@router.get("/latest", response_model=PriceResponse, summary="Поддерживаемые тикеры: btc_usd, eth_usd")
async def get_latest_price(db: DBSession, ticker: Ticker):
    """Возвращает последнюю цену указанного тикера"""
    return await service.get_latest_price(db, ticker)
...
async def get_price_by_date(
    ...
    return await service.get_price_by_date(db, ticker, from_ts, to_ts, limit, offset)
